...
|-------|------|----------|-------------|
| `account` | string | Yes | Account name matching `~/.cloudways/accounts.yml` |
| `server.id` | integer | Yes | Cloudways server ID |
| `server.ssh_user` | string | Yes | Default SSH user (e.g., `master_xxxxxxxxx`) |
| `server.ssh_host` | string | Yes | Server IP address |
| `server.label` | string | No | Human-readable server name |
| `server.cloud` | string | No | Cloud provider: `do` (DigitalOcean), `vultr`, `amazon` (AWS), `gce`, `linode` |
| `environments.<env>.app_id` | integer | Yes | Cloudways application ID |
| `environments.<env>.domain` | string | Yes | Primary domain for the environment |
| `environments.<env>.sys_user` | string | No | App file owner (Linux system user; home dir is `/home/<sys_user>/`) |
| `environments.<env>.ssh_user` | string | No | SSH/SFTP user for this environment (overrides `server.ssh_user`) |
| `environments.<env>.webroot` | string | No | Web root path relative to app root (default: `public_html`) |
| `environments.<env>.branch` | string | No | Git branch for this environment |
| `database.local_container` | string | Yes | Local Docker MySQL/MariaDB container name |
| `database.local_db_name` | string | Yes | Local database name |
| `database.url_replace_method` | string | Yes | `wp-cli`, `env-file`, or `sql-replace` |

> **Two-user distinction**: Cloudways environments have two separate user concepts:
>
> - **`sys_user`** — The Linux system user that owns application files (home directory: `/home/<sys_user>/`). Used by `reset_permissions` and rsync/scp operations targeting the app directory. Visible in `GET /app` response. Example: `unsvkhbwwr`.
>
> - **`ssh_user`** — The SSH/SFTP user account used for SSH connections and deployments. Can be the server master user (`master_xxxxxxxxx`) or an app-specific deployment user. Must have an SSH key attached for key-based authentication. Example: `bitbucket`.
>
> Set `environments.<env>.ssh_user` to override the server-level `server.ssh_user` for specific environments (e.g., staging uses a different deployment user than production).
